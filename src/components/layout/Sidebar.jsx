...
export function Sidebar({ mobileOpen = false, onClose }) {
  ...
  return (
    <aside className={`${mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} fixed inset-y-0 left-0 z-40 w-72 shrink-0 border-r border-slate-200 bg-white/90 p-5 backdrop-blur-xl transition-transform duration-200 md:static md:flex md:min-h-screen md:w-64 md:flex-col md:translate-x-0 dark:border-slate-800 dark:bg-slate-950/90`}>
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              onClick={onClose}
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium leading-5 transition ${
                active
